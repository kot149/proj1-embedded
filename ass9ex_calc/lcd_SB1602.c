...
inline void enableCursor(bool cursor_on, bool blink){
    uint8_t command[2] = {SB1602_COMMAND_SINGLE, 0b1100};
    if(cursor_on) command[1] |= 0b10;
    if(blink) command[1] |= 0b1;

    writeDataI2C(I2C3_BASE, SB1602_SLAVE_ADDRESS, command, 2);
}


void shiftCursor(int8_t shift){
    uint8_t command[2] = {SB1602_COMMAND_SINGLE, 0b10000};
    int8_t direction;
    uint8_t i;
    if(shift >= 0){
        command[1] |= 0b100;
    ...
    if((direction < 0 && cursor_pos == 0) || (direction > 0 && cursor_pos == 31)) return;

    writeDataI2C(I2C3_BASE, SB1602_SLAVE_ADDRESS, command, 2);
    cursor_pos += direction;

    if(direction > 0 && cursor_pos == 16){
        for(i = 0; i < 24; i++) writeDataI2C(I2C3_BASE, SB1602_SLAVE_ADDRESS, command, 2);
        cursor_pos = 16;
    } else if(direction < 0 && cursor_pos == 15){
        for(i = 0; i < 24; i++) writeDataI2C(I2C3_BASE, SB1602_SLAVE_ADDRESS, command, 2);
        cursor_pos = 15;
    }
}

void shiftCursor_n(int8_t shift){
    uint8_t i = 0;
    if(shift > 0) for(; i < shift; i++) shiftCursor(1);
    else for(; i < -shift; i++) shiftCursor(-1);
}
